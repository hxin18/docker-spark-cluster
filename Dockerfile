...
# install java, scala, zookeeper, hadoop, spark, sbt
ADD tar/* /usr/

# set environment variables
ENV JAVA_HOME /usr/jdk1.8.0_144
ENV SCALA_HOME /usr/scala-2.11.8
ENV ZOOKEEPER_HOME /usr/zookeeper-3.4.9
ENV HADOOP_HOME /usr/hadoop-2.7.4
ENV HADOOP_CLASSPATH ${JAVA_HOME}/lib/tools.jar
ENV SPARK_HOME /usr/spark-2.2.0-bin-without-hadoop
ENV SBT_HOME /usr/sbt
ENV PATH $PATH:$JAVA_HOME/bin:$SCALA_HOME/bin:$ZOOKEEPER_HOME/bin:$HADOOP_HOME/bin:$HADOOP_HOME/sbin:$SPARK_HOME/bin:$SPARK_HOME/sbin:$SBT_HOME/bin
